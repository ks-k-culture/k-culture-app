...
const TEAMS: Team[] = [
  ...
  { id: "13", name: "(주) 웨스트월드", category: "VFX, 색보정(DI)", hasPhone: true },
  { id: "14", name: "(주) 태양기획", category: "보조출연", worksCount: 1 },
  { id: "15", name: "(주) 디오스튜디오", category: "데이터(DIT)", worksCount: 2 },
  { id: "16", name: "(주) 팬 엔터테...", category: "제작", worksCount: 21 },
  { id: "17", name: "(주) 브로캐스팅", category: "보조출연", worksCount: 1 },
  { id: "18", name: "(주) 퍼펙트그립", category: "그립", worksCount: 1 },
];
...
function CategoryButton({ category, isSelected }: { category: Category; isSelected?: boolean }) {
  return (
    <Pressable className="w-1/4 items-center py-4">
      ...
      <Text className={`text-sm mt-2 ${isSelected ? "text-white" : "text-gray-400"}`}>{category.label}</Text>
    </Pressable>
  );
}
...
function TeamCard({ team }: { team: Team }) {
  return (
    <View className="w-1/3 items-center mb-6">
      ...
      <Text className="text-white font-semibold text-center text-sm" numberOfLines={1}>
        {team.name}
      </Text>
    ...
    <View className="flex-1 bg-black">
      <SafeAreaView className="flex-1" edges={["top"]}>
        ...
        <View className="flex-row items-center justify-between px-4 py-4">
          ...
          <Text className="text-white text-lg font-bold">팀 찾기</Text>
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          <View className="mx-4 bg-[#1A1A1A] rounded-2xl p-4 mb-4">
            ...
            <View className="flex-row bg-[#2A2A2A] rounded-xl p-1 mb-4">
              ...
              >
                <Text className={activeTab === "production" ? "text-black font-semibold" : "text-gray-400"}>
                  프로덕션
                </Text>
              </Pressable>
              <Pressable
                onPress={() => {
                  setActiveTab("postProduction");
                  setIsExpanded(false);
                }}
                className="flex-1 py-3 rounded-lg items-center"
                style={{ backgroundColor: activeTab === "postProduction" ? "white" : "transparent" }}
              >
                <Text className={activeTab === "postProduction" ? "text-black font-semibold" : "text-gray-400"}>
                  포스트프로덕션
                </Text>
              </Pressable>
            </View>
          ...
          <Pressable onPress={() => router.push("/team-register")} className="mx-4 mb-6">
            <View className="bg-orange-400 rounded-2xl px-5 py-4 flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Text className="text-white font-bold text-base ml-3">지금바로 팀을 등록해 보세요!</Text>
              </View>
